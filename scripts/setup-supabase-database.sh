...
echo "🗄️ Setting up Supabase Database for TauOS Platform..."

# Load environment variables
if [ -f "config/env.local" ]; then
    export $(cat config/env.local | grep -v '^#' | xargs)
fi

# Database connection details - properly escaped password
DB_URL="postgresql://postgres:Ak1233%40%405@db.tviqcormikopltejomkc.supabase.co:5432/postgres"
...
if [ $? -ne 0 ]; then
    echo "❌ Database connection failed. Trying alternative connection method..."
    ...
    psql "$DB_URL" -c "SELECT 1;" 2>/dev/null
    if [ $? -ne 0 ]; then
        echo "❌ Still failing. Let's try with environment variables..."
        export PGPASSWORD="Ak1233@@5"
        psql -h db.tviqcormikopltejomkc.supabase.co -U postgres -d postgres -c "SELECT 1;" 2>/dev/null
        if [ $? -ne 0 ]; then
            echo "❌ Database connection failed. Please check your credentials."
            echo "🔍 Troubleshooting:"
            echo "1. Verify the password is correct"
            echo "2. Check if your IP is whitelisted in Supabase"
            echo "3. Try connecting via Supabase CLI"
            exit 1
        fi
    fi
fi
...
echo "⚙️ Creating business functions..."
...
echo "✅ Business functions created successfully!"

# Test the connection
echo "🧪 Testing database connection..."

psql "$DB_URL" -c "SELECT COUNT(*) as organizations FROM organizations;" 2>/dev/null
if [ $? -eq 0 ]; then
    echo "✅ Database connection successful!"
else
    echo "❌ Database connection failed. Please check your credentials."
...
echo "   - Project ID: tviqcormikopltejomkc"
echo "   - Host: db.tviqcormikopltejomkc.supabase.co"
echo "   - Database: postgres"
echo "   - Tables: 8 tables created"
echo "   - Functions: 4 business functions"
echo "   - Sample Data: 2 organizations, 2 users"
echo ""
echo "🔗 Next steps:"
echo "1. Get your Supabase API keys from the dashboard"
echo "2. Update config/env.local with your API keys"
echo "3. Test the applications with the new database"
echo ""
