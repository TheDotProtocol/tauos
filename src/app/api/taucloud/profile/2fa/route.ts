...
import { logCloudActivity } from '@/lib/taucloud-files';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (action === 'setup') {
      ...
      const otpauthUrl = buildOtpAuthUrl(user.email, secret, 'Tau Cloud');
      return NextResponse.json({
        success: true,
        secret,
        otpauthUrl,
    ...
    if (action === 'enable') {
      ...
      await logCloudActivity(auth.userId, '2fa_enable', 'Enabled two-factor authentication', 'Via Web Portal');
    ...
    if (action === 'disable') {
      ...
      await logCloudActivity(auth.userId, '2fa_disable', 'Disabled two-factor authentication', 'Via Web Portal');
