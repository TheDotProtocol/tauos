...
import { Plus, Folder, Trash2, ArrowRight, Star, Clock } from 'lucide-react';
import {
      ...
      <div className="p-6 max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-400 text-sm mt-1">Persistent workspaces — synced to PostgreSQL when signed in.</p>
          </div>
        ...
        {showNew && (
          <div className="glass-strong rounded-xl p-6 mb-6 border border-cyan-500/20">
            ...
            <div className="space-y-3">
              <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Project name" className="w-full px-4 py-2 glass rounded-lg border border-white/10 text-white focus:border-cyan-500 focus:outline-none" />
              <textarea value={desc} onChange={(e) => setDesc(e.target.value)} placeholder="Description" rows={2} className="w-full px-4 py-2 glass rounded-lg border border-white/10 text-white focus:border-cyan-500 focus:outline-none resize-none" />
                ...
                <div className="flex items-center gap-4 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-xs text-gray-600 mt-1 flex items-center gap-2">
                      <Clock className="w-3 h-3" /> {p.files.length} files · {p.language}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <Link href="/developers/workspace" onClick={() => setActiveProjectId(p.id)} className="btn-primary text-sm py-2 px-4">
                    Open <ArrowRight className="w-4 h-4" />
                  </Link>
                  {p.id !== 'default' && (
                    <button onClick={() => handleDelete(p.id)} className="p-2 text-gray-500 hover:text-red-400 rounded-lg">
