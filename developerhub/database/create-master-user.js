...
/**
 * TauCore™ Master User Creator
 * Creates the master user with the correct schema
 */
...
async function createMasterUser() {
    try {
        ...
        console.log('\n3️⃣ Creating user preferences...');
        await pool.query(`
            INSERT INTO user_preferences (user_id, theme, language, notifications_enabled, created_at, updated_at)
        ...
        await pool.query(`
            INSERT INTO user_profiles (user_id, bio, location, website, created_at, updated_at)
            VALUES ($1, $2, $3, $4, NOW(), NOW())
        `, [
            newUser.id,
            'CEO and Founder of TauOS - The Future of Computing',
            'Malaysia',
            'https://tauos.org'
        ]);
