...
pub mod helpers {
    ...
    pub fn sidebar() -> GtkBox {
        TauWidgets::create_sidebar()
    }
    
    /// Create a simple sidebar item
    pub fn sidebar_item(icon_name: &str, label: &str) -> GtkBox {
        TauWidgets::create_sidebar_item(icon_name, label, false)
    }
    
    /// Create a simple list row
    pub fn list_row(title: &str, subtitle: Option<&str>) -> ListBoxRow {
        TauWidgets::create_list_row(title, subtitle)
    }
} 