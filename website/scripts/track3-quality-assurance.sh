...
test_core_components() {
    ...
    for component in "${components[@]}"; do
        if [ -f "$PROJECT_ROOT/core/$component/target/release/$component" ]; then
...
test_gui_applications() {
    ...
    local apps=("settings" "launcher" "splash")
    local test_results=()
    
    for app in "${apps[@]}"; do
        if [ -f "$PROJECT_ROOT/gui/$app/target/release/$app" ]; then
...
test_desktop_applications() {
    ...
    for app in "${apps[@]}"; do
        if [ -f "$PROJECT_ROOT/apps/$app/target/release/$app" ]; then
...
test_website_integration() {
    ...
    local website_files=(
        ...
        "website/src/app/governance/page.tsx"
        "website/src/app/legal/page.tsx"
        "website/src/app/careers/page.tsx"
    )
    local test_results=()
...
test_security() {
    ...
    local error_handling_files=("core/tau-session/src/main.rs" "gui/settings/src/main.rs" "apps/taustore/src/main.rs")
    for file in "${error_handling_files[@]}"; do
        if [ -f "$PROJECT_ROOT/$file" ] && grep -q "Result\|Option" "$PROJECT_ROOT/$file"; then
            print_status "Error handling found in: $(basename "$file")"
            security_checks+=("PASS:error_handling_$(basename "$file")")
        else
            print_warning "Error handling missing in: $(basename "$file")"
            security_checks+=("FAIL:error_handling_$(basename "$file")")
        fi
    done
