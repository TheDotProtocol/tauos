...
echo "🚀 Setting up Supabase for TauOS Platform..."
...
STORAGE_BUCKET=tauos-storage
STORAGE_REGION=us-east-1

# Pricing Configuration
FREE_STORAGE_LIMIT=5368709120
BASIC_PLAN_STORAGE=107374182400
PRO_PLAN_STORAGE=1073741824000
ENTERPRISE_STORAGE=10737418240000

# Domain Configuration
ALLOWED_DOMAINS=tauos.org,arholdings.group
EOF
...
CREATE POLICY "Organizations are updatable by organization owners" ON organizations
    FOR UPDATE USING (auth.uid() IN (
        SELECT user_id FROM users WHERE organization_id = organizations.id AND role = 'owner'
    ));
...
CREATE POLICY "Users can create emails in their organization" ON emails
...
cat > supabase/migrations/002_storage_buckets.sql << 'EOF'
-- Create storage buckets for file uploads
INSERT INTO storage.buckets (id, name, public) VALUES
('tauos-files', 'tauos-files', false),
('tauos-avatars', 'tauos-avatars', true),
('tauos-logos', 'tauos-logos', true);

-- Storage policies
CREATE POLICY "Users can upload files to their organization bucket" ON storage.objects
    FOR INSERT WITH CHECK (
        bucket_id = 'tauos-files' AND 
...
echo "⚙️ Creating business logic functions..."

cat > supabase/migrations/003_business_functions.sql << 'EOF'
...
echo "2. Get your API keys from the Supabase dashboard"
