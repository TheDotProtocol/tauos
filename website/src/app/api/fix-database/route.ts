...
export async function POST(request: NextRequest) {
  try {
    console.log('🔧 FIXING DATABASE - CREATING MISSING TABLE');
    
    // Use the same connection as the API (which works for users table)
    const pool = new Pool({
    ...
    return NextResponse.json({
      success: true,
      message: 'Table created successfully in API database',
